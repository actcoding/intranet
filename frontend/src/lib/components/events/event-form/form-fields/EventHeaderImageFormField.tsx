...
import { eventDetachFile, uploadEventFileAction } from '@/lib/actions/events'
...
export const EventHeaderImageFormField = () => {
    const [file, setFile] = useState<File>()
    const { toast } = useToast()
    const router = useRouter()
    const { event } = useEvent()
    const current = event.headerImage?.data

    const onChange = useCallback(
        async (files: File[]) => {
            if (current) {
                await eventDetachFile(event.id, current.id)
            }
    ...
    return (
        ...
        >
            <FileSelectorTrigger asChild>
                <NewsHeaderImageUploadButton file={current} />
            </FileSelectorTrigger>
            <FileSelectorContent>
                <FileSelectorHeader>
                    <FileSelectorTitle>Titelbild ändern</FileSelectorTitle>
                </FileSelectorHeader>
                <FileSelectorBody>
                    <FileSelectorInput />
